...
```{r, message = TRUE, results='asis', eval=(opts_knit$get('rmarkdown.pandoc.to') == 'latex'), echo = FALSE} 
...
flags_disp_used <- c("individual", "groups_1", "groups_2", "uniform_1", "uniform_2")[5] #  |> rev() |> first()
        ...
        for(y0_flag in flags_y0_used) {
          ...
          for(disp_value in values_disp_used) {
            ...
            if(render_hex) {
              plot_name <- "plot_hex"
              plot_curr <- plots_row[[plot_name]]            
              if( (is.null(plot_curr) | render_plots_force | TRUE) & render_plots) { ## Generate and append plot
                print("Plotting Hex")
                scatter_list <- list()
