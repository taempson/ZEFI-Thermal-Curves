...
To understand this, I want to transform the flat prior on `b0` into the corresponding prior on `x0`.
In order to do so, I am following`Proposition B` on p. 60 of Rice (1995) stats book.

Specifically, if `b0 = -1/(xmax - x0)` such that `y = y0 * b0 * (max(x, x0)- xmax)`

Then,
$f_{X0}(x0) = f_{B0}( g^{-1}(x0)) |\frac{d}{d x0} g^{-1}(x0)$ where

- $g^{-1}(x0) = -1/(x_{\max} - x0) = b0$
- $|\frac{d}{d x0} g^{-1}(x0)| = | \frac{d}{d x0}(-1/(x_{\max} - x0)$
  $ = |-\frac{1}{(x_{\max} - x0)^2}| = \frac{1}{(x_{\max} - x0)^2}$
- Assuming a flat prior means $f_{B0}(b0) = C$
- Thus, given a flat prior on $b0$, the corresponding prior on $x0$ is $f(x0) = C \frac{1}{(x_{\max} - x0)^2}$ which is not what I expected since it favors higher values of $x0$
...
stan_two_piece_b0_func <- paste0(" real  two_piece(real x, real b0, real y0) {
 real xmax = ", xmax, "; ## paste in value for xmax\n
 real x0 = y0/b + xmax;
 real y = - b * (xmax - max(x, x0));
 return(y);
 } ")


models <- c("x0_piecewise", "b0_piecewise") #, "asymptotic")
...
disp_prior_list <- c(0.25, 0.5, 2, 4, 8, 16, 32)
...
infile_tbl <- file.path(output_dir, paste0("fit_tbl_2023-03-07.Rda"))
...
flags_x0_used <- c("individual", "uniform_1", "groups_1") #
flags_y0_used <- c("individual")
disp_prior_used <- disp_prior_list
models_used <- "x0_piecewise"

warmup <- 15000 #floor(3/4 * iter)
iter <- 25000
...
for(model in models_used) {  
    switch(model,
           ...
           "x0_piecewise" = {stan_func <- stan_two_piece_x0_func},
           "b0_piecewise" = {stan_func <- stan_two_piece_x0_func} ## don't use b0 code, instead use appropriate prior
           )
      ...
      for(disp_prior in disp_prior_used) {
        ...
        if(run_fits) {
          ...
          prior_core <- switch(5,
                              ...
                              set_prior(paste0("constant(", disp_prior, ")"), class = "disp") ## Making disp a constant oesn't really help.
                               ) +
              prior(uniform(25, 45), lb = 25, ub = 45, nlpar = "x0")
            
##              switch(model,
##                     x0_piecewise = prior(uniform(25, 45), lb = 25, ub = 45, nlpar = "x0"),                   
                     ## Priors for b0
                     ## Smallest slope: if x0 = 25; then b0 = - 1/(20) < -1/20 = -0.05 
                     ## Most extreme slope: if x0 = xmax-0.5; then b = -1/0.5 > -2
                     # b0_piecewise = prior(uniform(-2, -0.05), lb = -2, ub = -0.05, nlpar = "b0")
##                     b0_piecewise = NULL
##                     )
          ...
          y0_priors <- switch(y0_flag,
                              ...
                              groups_2 = ,
                              individual = switch(2,
                                                  prior(normal(150, 1000), nlpar = "y0", lb = 10, ub = 1000),
          ...
          ## This if() needs to be revised since fit_tbl is defined (but empty)
          ## Could test if fit_tbl[[curr_row, "fit"]][[1]] is NA
          if(!exists("fit_tbl") | force_load) {
          ...
          hist <-  stan_hist(fit_stan, pars = variables(fit), bins = 30) +
                     ggtitle(desc_short)
          print(hist)
          ## Save hist to file
          ggsave(filename = paste0("histogram_", filename_desc, ".pdf"), path = file.path(output_dir, "figures"), dpi=300)
