...
data_ind <- data_ind %>% filter(temp < 38) %>%
    group_by(male) %>% mutate(y0_obs = mean(motif_count), phi_ind = var(motif_count)/y0_obs)
...
data_sim <- data_tmp %>% rowwise() %>% mutate(motif_count = sim_counts(temp, y0, tau0, tmax = tmax, phi = phi))
