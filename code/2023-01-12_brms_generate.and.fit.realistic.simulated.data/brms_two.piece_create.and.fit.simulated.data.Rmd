...
hist_x0_vals <- p + 
    aes(x0, fill = factor(x_group)) +
  geom_histogram(binwidth = 0.75) 
last_plot()

hist_y0_vals <- p +
    aes(y0, fill = factor(y_group)) +
    geom_histogram(binwidth = 25)
last_plot()

scatter_x0_vs_y0 <- p +
  aes(x = x0, y = y0, color = interaction(x_group, y_group, sep=':')) + 
  geom_point() +
...
plot <- ggplot(data) +
  ...
  geom_point() +
  facet_grid(rows = vars(x_group), cols = vars(y_group)) #, scales = "free_y")
last_plot()
...
for(x_flag in flags_x) {
    for(y_flag in flags_y) {
        ...
        x_form <- switch(x_flag,
                         ...
                         groups_1 = formula(x0 ~ (1|| male)), # no need for x_group when there's only 1 group
                         groups_2 = formula(x0 ~ 1 + (x_group|| male))
                         )
        
        y_form <- switch(y_flag,
                         individual = formula(y0 ~ -1 + male), 
                         groups_1 = formula(y0 ~ (1|| male)),
                         groups_2 = formula(y0 ~ 1 + (y_group|| male))
                         )
        nlform <- bf(
