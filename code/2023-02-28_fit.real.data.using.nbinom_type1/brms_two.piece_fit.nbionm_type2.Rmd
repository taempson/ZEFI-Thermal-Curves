...
infile_tbl <- file.path(output_dir, paste0("fit_tbl_2023-03-08.Rda"))
...
for(model in models_used) {  
    switch(model,
           ...
           "s0_piecewise" = {stan_func <- stan_two_piece_s0_func} ## don't use s0 code, instead use appropriate prior
           )
    ...
    for(y0_flag in flags_y0_used) {
      for(disp_prior in disp_prior_used) {
        ...
        if(run_fits) {
          ...
          disp_priors <- set_prior(switch(5,
                               paste0("exponential(", disp_prior, ")"), ## Similar to one used in earlier work with stan
          ...
          s0_prior <- prior(uniform(0.02, 1), lb = 0.02, ub = 1, nlpar = "s0")

          s0_priors <- if(is.null(x0_prior)) {
            s0_prior
          } else {
            x0_prior %>% mutate(nlpar = "s0") +
              s0_prior
          }
        ...
        if(is.na(list(fit))) {
          ...
          print(desc)
          print("Threshold Prior Information")
