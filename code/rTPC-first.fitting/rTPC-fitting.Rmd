...
```
- Create `nls_multstart_allfits()` to get all model fits.

```{r}

nls_multstart_allfits <- function (formula, data = parent.frame(), iter, start_lower, 
...
weights_index <- 1:1
data_indexes <- 1:2
data_descriptions <- c("Unaugmented", "song_count at 46C Augmented")
...
for(data_index in data_indexes ){
    ...
    data <- data %>% rename(rate = song_prop) %>%
    ...
    for(model_str in model_set$model) {
        ...
        for(wts in weights_index) {
            ...
            fits_all <- nls_multstart_allfits(formula = formula,
                                 data = data,
                                 iter = iter, 
