...
song_stats <- filter(stats_40, !is.na(var))
song_stats

for(filter_high_var in c(FALSE, TRUE)){
    for(filter_low_mean in c(FALSE, TRUE)){

        print(paste0("Mean vs Var; filter high var: ", filter_high_var, "; filter low mean: ", filter_low_mean))
        ...
        ga <- grid.arrange(g0, g2, g4, g5,
                     ncol=2,
                     top=textGrob(
                         paste0("Mean vs. Var: Filter High Var = ", filter_high_var,
                                ", Low Mean = ", filter_low_mean, "\n temp_target = 40C"),
                         gp=gpar(fontsize = 11))
                     )
        ga
        
       dev.print(device = pdf, file = file.path(output_dir, paste0("mean.vs.var_filter.high-", filter_high_var, ".filter.low.mean-", filter_low_mean, ".pdf")))
...
for(filter_high_var in c(FALSE, TRUE)){
    for(filter_low_mean in c(FALSE, TRUE)){
        ...
        print(paste0("song_count vs. trial; filter.high: ", filter_high_var, "; filter.low.mean: ", filter_low_mean))
        ...
        fit_g2 <- lm( song_count ~ 1 + trial, data = data_tmp)
        print(summary(fit_g2))
        ...
        ga <- grid.arrange(g2, g3, g4, as_ggplot(legend),
                           ncol=2,
                           top=textGrob(
                               paste0("song_count vs. trial: Filter High Var = ", filter_high_var, ", Low Mean = ", filter_low_mean, "\n temp_target = 40C"),
                               gp=gpar(fontsize = 11))
                           )
        ga
        
        dev.print(device = pdf, file = file.path(output_dir, paste0("song_count.vs.trial_filter.high-", filter_high_var, ".filter.low.mean-", filter_low_mean, ".pdf")))
    }
}
...
## See: https://www.statforbiology.com/nonlinearregression/usefulequations
    ...
    if(FALSE) {
        ...
        getInitial(song_count ~ SSasymp(trial, Asym, resp0, lrc), data = data_tmp)  
        ar <- nls(song_count ~ SSasymp(trial, Asym, resp0, lrc), data = data_tmp)

        ## Try using song_count_plus_1
        getInitial(song_count_plus_1 ~ SSasymp(trial, Asym, resp0, lrc), data = data_tmp)  
        ar <- nls(song_count_plus_1 ~ SSasymp(trial, Asym, resp0, lrc), data = data_tmp)
    }
    ...
    formula <- song_count ~ 1 + Mult(Const(-1), Exp(1 + trial))
    gnm_poisson <- gnm(formula = formula,
                       data = data_tmp,
                       ## start = c(50, -1, -0.1),
                       family = poisson(link = "identity"),
                       ##weights = 1/song_count_plus_1,
                       trace = TRUE,
                       iterMax = 5000
                   )
