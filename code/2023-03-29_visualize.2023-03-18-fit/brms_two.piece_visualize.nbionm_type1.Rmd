...
if(!exists("two_piece") | force_expose_functions) expose_functions(fit_tbl[[1, "fit"]], vectorize = TRUE, show_compiler_warnings = FALSE)
...
plot_pred<- function(data_pred, male_vec, desc, data_obs = NULL) {
  ...
  y_max <- max(data_pred_tmp$.prediction)*1.1
  plot_tmp <- plot_tmp +
    ylim(0, y_max) +
    facet_wrap(vars(male)) +
    labs(title = title, subtitle = desc)
