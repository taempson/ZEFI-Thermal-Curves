...
for(row_index in 3:3) { #nrow(fit_tbl)) {
  ...
  if(print_mcmc_pairs) {
    ...
    for(male in male_vec) {
      ...
      tmp_n <- sqrt(length(plots_tmp))
      plot_index <- 1:tmp_n
      ## drop terms on the diagonal
      drop_list = matrix(x = plot_index, y = plot_index)
      plot_index <- plot_index[-(drop_list)]
