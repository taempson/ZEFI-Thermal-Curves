...
x0_group_means <- c(39-2, 39+2) ## values based on mean = 39, sd = 1 and n_sd = 2
y0_group_means <- summary_stats$y0_bar + c(-2, 2)*summary_stats$y0_sd
...
data <- data_sim %>% filter(x_flag == x_flag_data & y_flag == x_flag_data, sampling_dist =="nb", n_reps == 32) ## could try to add column with strings for faced_grid labelling mutate(factor(x0_group, labels = c("mean(x_0) =  
```
...
data_parms <- data[, c("index", "x0", "x0_group", "y0", "y0_group")] %>% unique()
