...
if(TRUE) {
    ...
    x0_sim <- sim_parms(flag = x_flag, n_male = n_male, mean = 39, sd = 2)  %>%
        rename(x0 = parm, xx = grp)

    x0_y0_sim <- full_join(x0_sim, y0_sim, by = "id") %>%
        relocate(id) ## move id to first column


    data_parms <- crossing(x0_y0_sim, temp_target = c(30, 38, 40, 42, 44), rep = 1:3 )
    ...
    data_sim <- data_parms %>% mutate(
                                   motif_count_qpoisson = sim_qpoisson_counts(temp_mean, y0, x0, tmax, phi),
                                   motif_count_nb = sim_nb_counts(temp_mean, y0, x0, tmax, theta))
