...
if(interactive()) {
  ...
  file.path("output", "render")
  infile <- last(dir(file.path("input", "tibbles"), "fit_tbl.*"))
  infile_tbl <- file.path("input", "tibbles", infile)
}
...
for(name in names(fit_tbl)[1:8]) {
  ...
  # access via get()
  print(paste0(var, ": ", paste0(get(var), sep = "\n\t")))
}
...
for(row_index in 1:nrow(fit_tbl)) {
  ...
  if(print_prior_summary) {
    ...
    if(FALSE) { 
      print(head(prior_summary(fit_brms)) # %>% filter(nlpar!="y0"))
      } else {
        n_rows_half <- 4
        tmp_prior <- prior_summary(fit_brms)
        #class(tmp_prior) <- "data.frame"
        #tmp_prior <- tmp_prior %>% select(-source)
        rows_print <- unique(c(1:n_rows_half, nrow(tmp) - (n_rows_half):0))
  ...
  ## ensure we can make a pairs plot for each male 
  if(all(unique(male_instance) == 2)) {
    ...
    if(print_mcmc_pairs) {
      ...
      for(male in male_vec) {
        ...
        if(length(plots_tmp) == 4) {
          plot_index <- c(2,3)
        }
        if(length(plots_tmp) == 9) {
          plot_index <- c(2, 3, 4, 6, 7, 8)
        }

        for(i in plot_index) {
          # pars = c(vars_male): list of 4 ggplot objects diagonal plots are 2 and 3
          plots_tmp[[i]] <- plots_tmp[[2]] +
            stat_density_2d(color = "black",
                            size = .25,
                            bins = 5)
        }
        
        pairs_tmp <-bayesplot_grid(plots = plots_tmp) ## Change font size
        pairs_list[[male]] <- pairs_tmp
      } 

      p <- plot_grid(plotlist = pairs_list,
                     ncol = 2)
      plot_pairs_list <- plot_grid(title_row, p,  ncol = 1,
                                   rel_heights=c(0.06, 1))


      print(plot_pairs_list)
      filename <- paste0("plot-pairs_", filename_desc, ".png")
  ...
  if(print_mcmc_scatter) {
    ...
    for(male in male_vec) {
      ...
      scatter_tmp <-  mcmc_scatter( #was mcmc_scatter
        fit_stan_rename,
        pars = c(vars_male) #, vars_Intercept),
        #off_diag_args = list(size = 1, alpha = 0.35), # only used if `off_diag_fun = "scatter"`
        #off_diag_fun = c("hex")
        )  #+
      # stat_density_2d(color = "lightgray", size = .5, bins = 5) # doesn't work with mcmc_pairs
    ...
    filename <- paste0("plot-scatter_", filename_desc, ".png")
    ggsave(filename = filename, path = file.path(output_dir, "figures"),
           width = 8, height = 11, units = "in",
           scale = 1,
           dpi=300,
  ...
  if(print_mcmc_pairs_group) {
    ...
    print(plot_pairs_list2)
        ...
        filename <- paste0("plot-pairs2_", filename_desc, ".png")
    ggsave(filename = filename, path = file.path(output_dir, "figures"),
           width = 8, height = 11, units = "in",
           scale = 1,
           dpi=300,
...
for(curr_row in 1:curr_row_max) {
  ...
  last_plot()
  filename <- paste0("data.and.epred-vs-x_", filename_desc, ".png")
  ggsave(filename = filename, path = file.path(output_dir, "figures"),
         width = 8, height = 11, units = "in",
         scale = 1,
         dpi=300,
...
for(curr_row in 1:curr_row_max) {
  ...
  filename <- paste0("data-simulated-vs-x_", filename_desc, ".png")
  ggsave(filename = filename, path = file.path(output_dir, "figures"),
         width = 8, height = 11, units = "in",
         scale = 1,
         dpi=300,
...
filename <- paste0("data-epred-vs-temp_", filename_desc, ".png")
ggsave(filename = filename, path = file.path(output_dir, "figures"),
       width = 8, height = 11, units = "in",
       scale = 1,
       dpi=300,
