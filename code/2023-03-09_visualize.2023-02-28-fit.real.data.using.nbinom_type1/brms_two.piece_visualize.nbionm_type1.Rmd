...
## add .epred and other colums

data_epred <- data_fit %>%
    group_by(male) %>%
    data_grid(x = seq_range(c(20, 47), n = 51)) %>%
    add_epred_draws(object = fit, ndraws = 2000)
...
for(male in male_vec) {
    ...
    tmp_data <- data_fit %>% filter(male == !!male)
    tmp_y <- tmp_data$y
    tmp_epred <- data_epred %>% filter(male == !!male)
    
    plot_tmp <- ggplot(data = tmp_epred,
                       aes(x = x, y = tmp_y)) +
        ylim(0, max(tmp_y)*1.1) +
        stat_lineribbon(aes(y = .epred)) +
        scale_fill_brewer(palette = "Greys") +
    ...
    print(plot_tmp)
    plot_model_list[[male]] <- plot_tmp
}

gridExtra::grid.arrange(grobs = plot_model_list)

cowplot::plot_grid(plotlist = plot_model_list,
                   ncol = 3)
```

## Combine Scatter Plots and Model Predictions
