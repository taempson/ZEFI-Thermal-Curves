...
g2 <- ggplot(song_count_round_2 %>% select(-song_count)) +
    aes(x=mean, y = sd) + 
    geom_point(aes(color = male)) +
    geom_smooth(method='lm', formula= y~x) +
    labs(title = "Mean vs. sd for Round 2: temp_target = 40")

g3 <- ggplot(song_count_round_2 %>% select(-song_count)) +
    aes(x=mean, y = var) +
    geom_point(aes(color = male)) +
    geom_smooth(method='lm', formula= y~x) +
...
g4 <- ggplot(song_count_round_2 %>% select(-song_count)) +
    aes(x=mean, y = var) +
    geom_point(aes(color = male)) +
...
male_coef <-  coef(glm_poisson) %>% keep(str_detect(names(.), 'male'))
