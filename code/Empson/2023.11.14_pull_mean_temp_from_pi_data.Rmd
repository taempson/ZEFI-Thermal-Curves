...
# Load file with miscalaneous information about each bird. 
I think I made this sheet by copy and pasting things from the Mastersheet.
```{r}
misc.info <- read.csv("~/Desktop/MDSHrna_male_summary2023.11.14.csv") %>% 
    clean_names()
```

# Tried to extract from data loggers
- The file names don't match notes, so it didn't work. 
```{r}
...
- Since extracting from the insertable temp loggers doesn't seem to work for now, I'm going to instead extract the temp from the camera's built in temperature recording.
- Con: This method is only from while the camera was recording. So its not consistent if the camera was turned on too early or late. 
```{r}
...
```{r}
#write_csv(done, paste("misc.data.w.temp_", Sys.Date(), ".csv"))
```
