...
disp_prior_list <- c(1, 2, 8, 16, 32, 64)
...
for(model in models[[1]]) {
    ...
    switch(sampling,
           "nbinom_type1"= {
             ...
             iter <- 15000 # could try <100000
             warmup <- floor(3/4 * iter)
             thin <- 1
  ...
  for(x0_flag in flags_x0_used) {
    for(y0_flag in flags_y0_used) {
      for(disp_prior in disp_prior_list[[5]]) {
      ...
      nlform <- bf(
        ...
        save_model <-paste0(paste("brms", model, sampling, x0_flag, y0_flag, sep="_"), ".stan")
    ...
    ## model in global environment
        ...
        fit_stan <- fit$fit
        print(stan_hist(fit_stan, pars = c("b_y0", "disp") ))
        print(stan_hist(fit_stan, pars = c("b_x0", "disp")))
