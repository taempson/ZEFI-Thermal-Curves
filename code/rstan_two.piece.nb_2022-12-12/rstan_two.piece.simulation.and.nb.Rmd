...
plot(model, pars = c("x0", "y0") )

fit_poisson_separate <- model
...
pars_sim <- data_sim %>% select(male_index = male, y0, x0 = tau0) %>% distinct()
...
pars_sim <- data_sim %>%
    select(male_index = male, y0, x0 = tau0) %>%
    distinct() %>%
#    mutate(type = "sim") %>%
...
## Get basic numbers
pars_tmp <- summary(model, pars = pars, probs = NULL)
...
ggplot(data = pars_combined) +
    ...
    geom_errorbar(aes(x = sim, ymin = est - 2 * sd, ymax = est + 2 * sd)) +
    geom_abline(aes(intercept = 0, slope = 1)) +
