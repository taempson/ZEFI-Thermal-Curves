...
output_dir_list <- file.path(output_dir, c("stan", "figures", "cmdstanr"))
                         
dir.create(output_dir_list, showWarnings = FALSE, recursive = TRUE, mode = "0777")
cmdstanr_dir <- file.path(output_dir, "cmdstanr")

options("cmdstanr_write_stan_file_dir" = cmdstanr_dir)
...
for(temp_threshold in 26:45) {
  tmp <- data_ind %>% group_by(male) %>% filter(temp < temp_threshold) %>% summarize(mean = mean(motif_count), sd = sd(motif_count), cv = sd(motif_count)/mean(motif_count))
  mean <- mean(tmp$mean)
  sd <- sd(tmp$mean)
...
pairs(cumulative_count_vs_temp[[39]] %>% select(- male))
...
cowplot::plot_grid(plotlist = list(hist, hist_log))

```

Go with `mean = 125` and `sd = 125 * 4 = 500`
...
values_disp <- switch(2,
                      c(0.1), # 0.125 is a good value
...
flags_disp_used <- c("individual", "groups_1", "uniform_1") #  |> rev() |> first()
models_used <- c("one_piece", "two_piece") #, "asymptotic") #c("one_piece", "two_piece") #, "two_piece")
shape_y0_prior <- "flat" # flat or normal

## Should remove this since males were identified with wrong llik function
## males whose E(disp) in one_piece is < 4
males_keep <- c("T234", "T236", "T237", "T244", "T247", "T257", "T260")

fit_index <- 0

#for(filter_male in c(FALSE)) {
      ...
      for(x0_flag in flags_x0_used) {
        ...
        for(y0_flag in flags_y0_used) {
          ...
          for(disp_value in values_disp_used) {
            ...
            stan_model_name <- sub(filename_desc, "_disp-prior-[0-9.]+_", "_")
            ...
            if(run_fits) {
              ...
              x0_priors <- prior(uniform(25, 43), lb = 25, ub = 43, nlpar = "x0") + x0_prior
              ...
              disp_priors <- switch(disp_flag,
                                    ...
                                    groups_1 = set_prior(prior_string,
                                      ...
                                      prior(normal(10, 20), class = "sd", dpar = "disp", lb = 0, ub = 20),
                                    groups_2a = NULL,
                                    groups_2b = NULL,
              ...
              stan_code <- file.path(output_dir,
                                     "stan", paste0(stan_model_name, ".stan"))
