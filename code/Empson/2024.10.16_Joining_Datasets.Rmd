...
Tamara.rough.counts<- read_xlsx("All_Directed_Song_Counts.xlsx") %>% 
    clean_names() %>% 
    mutate(`date`=as.Date(`date`, format = "%Y-%m-%d"))%>% 
    mutate(male=case_when(male=="T321"~"T231", #cleaning error/typos found after checking with raw datasheets
                          male!="T321"~male),
           `date`= case_when(`date` == "2022-04-16"~`date`-1,
                           T~`date`),
           `date` = case_when(`date` == "2022-06-24" & male=="T244" ~ `date`+1,
        TRUE ~ `date`),
        `date` = case_when(`date` == "2022-02-13" & male=="T230" ~ `date`+10,
        TRUE ~ `date`))

Tamara.rough.durations<- 
Tamara.counts<- Tamara.rough.counts %>% left_join(Tamara.rough.durations, by="clipped_file_name")
