---
title: "Piecewise Regression with NB Error on Real Data using BRMS"
...
## for calculation of 'size_bar' see note in `Set Up Simulate Data`
summary_stats <- data_ind %>% ungroup() %>%  summarize(y0_bar = mean(y0_simple.est), y0_sd = sd(y0_simple.est), phi_bar = median(phi_ind), size_bar = y0_bar^2/(y0_sd^2 - y0_bar))
...
We do this above in summary_stats `size_bar = y0_bar^2/(y0_sd^2 - y0_bar)`


```{r}
sim_nb_counts <- function(temp, y0, x0, tmax, size) {
...
## Should use estimates from 
set.seed(2021)
x0 <- rnorm(n_male, 39, 2) %>% round(1)
## Match y0 and x0 to males
data_tmp <- data_tmp %>% mutate(x0 = x0[.data$male])
    
data_sim_qpoisson <- data_tmp %>% rowwise() %>% mutate(motif_count = sim_qpoisson_counts(temp, y0, x0, tmax = tmax, phi = phi))

data_sim_nb <- data_tmp %>% rowwise() %>% mutate(motif_count = sim_nb_counts(temp, y0, x0, tmax = tmax, size = size))
...
## Reference: Negative Binomial via `rstan`
            ...
            switch(model,
                   ...
                   brms = {
                       ...
                       my_priors <- prior(normal(150, 200), nlpar = "y0", lb = 0.1) +
                         ...
                         prior(uniform(0.1, 4), lb = 0.1, ub = 4, class = "sd", group = "xx", nlpar = "x0") + 
                       ...
                       nlform <- bf(y ~  0 - (xmax - fmax(x0, x)) * (0 - y0)/(xmax - x0),
                                    shape ~ 1,
                                    x0 ~ 1 + (1|xx),#|index),
                                    y0 ~ 1 + (1|yy),#|index),
                                    nl = TRUE)
