...
sim_parms <- function(flag = "uniform_1", n_male = 10, mean_global = 100, sd = 10, min = 10) {

    mean_low <- mean_global - 3 * sd
    mean_high <- mean_global + 3 * sd
    ...
    switch(flag,
           ...
           groups_1 = { ## note syntax differs from model usage (this would 'be pooled')
        ...
        mutate(parm = round(parm, 0)) %>%
        mutate(parm = pmax(10, parm)) %>%
...
data_tmp <- data_ind[, c("male", "index", "temp", "y0_simple.est")] ##%>%
##    mutate(male = droplevels(male)) %>%
...
if(TRUE) {
    ...
    x0_sim <- sim_x0_or_y0(flag = x_flag, n_male = n_male, mean = 39, sd = 2)
...
data_tmp <- data_tmp %>% mutate(x0 = x0[.data$index])
    
data_sim_qpoisson <- data_tmp %>% rowwise() %>% mutate(motif_count = sim_qpoisson_counts(temp, y0, x0, tmax = tmax, phi = phi))

data_sim_nb <- data_tmp %>% rowwise() %>% mutate(motif_count = sim_nb_counts(temp, y0, x0, tmax = tmax, theta = theta))
