...
fit_tbl_initiate_crossed <- FALSE
...
flags_disp <- c("uniform_1", "groups_1", "individual", "uniform_2")
...
render_plots_force <- FALSE ## Force regeneration of plots if they already exist
display_plots <- TRUE  ## Display plots to local device?
save_plots_file <- TRUE  ## Save plots to file?
save_plots_tibble
rerender_plots <- FALSE ## Option to force rerendering of plot
run_fits <- TRUE
save_fits <- TRUE
skip_existing_fits <- FALSE
force_load <- FALSE  #reload fit_tbl even if it already exits
print_get_prior <- FALSE ## Print generic priors, prior to fit.
print_prior_summary <- FALSE  ## Print actual priors, post fitting
...
flags_disp_used <- c("individual", "groups_1", "uniform_1", "uniform_2")[3:4] #  |> rev() |> first()
  ...
  for(model in models_used) {
    ...
    switch(model,           
           two_piece = {
             ...
             iter <- warmup + 10000
             adapt_delta <- 0.99 ## increasing from 0.9 to 0.99 didn't help eliminate divergence
             thin <- 5
           },
           one_piece = {
        ...
        for(y0_flag in flags_y0_used) {
          ...
          for(disp_value in values_disp_used) {
            ...
            if(run_fits) {
              ...
              fit_prerun <- fit_tbl[[curr_row, "fit"]][[1]]
              if(!skip_existing_fits | !is_brmsfit(fit_prerun)) {
                  ...
                  data <- switch(switch_male,
                                 ...
                                 "double" = {
                                   mutate(
                                     data,
                                     tmp_group =
                                       if_else(male %in% male_disp_high, 2, 1))
                                 },
                                 "individual" = {
                                   mutate(data, tmp_group = male)
                                 },
                                 NA
                                 ) %>%
                ...
                for(flag_prefix in c("x0", "y0", "disp") ) {
                  ...
                  string_formula <- switch(flag,
                                           ...
                                           uniform_2 = paste0(flag_prefix, " ~ 0 + ", flag_prefix, "_group"),
                                           ...
                                           groups_2 = paste0(flag_prefix, " ~ 0 + Intercept + (1|male) + ", flag_prefix, "_group"),
              ...
              if(!exists("fit_tbl") | force_load | nrow(fit_tbl) ==1) {
            ...
            fit_stan_rename <-
              ...
              setNames(gsub("b_", "", names(.)) %>%
                         gsub("(x0|phi|y0)_male[0-9]{3})", "\\2_\\1", .) %>%
                         gsub("__", "_", .) %>%
                         gsub("r_male_(x0|phi|y0)\\[(Tplo[0-9]{3}),Intercept\\]", "\\2_\\1_r", .) %>%
                         gsub("\\.", " ", .))

            ##  Get plots for current fit settings
            plots_row <- fit_tbl[[ curr_row, "plots"]]

            ## indicate pXlot name
            plot_name <- "plot_trace"            
            plot_curr <- plots_row[[plot_name]]            
            if(is.na(plot_curr) | render_plots_force) { ## Generate and append plot
            ...
            plot_trace_and_violin <- plot_grid(plotlist = list(plot_trace, plot_violin),
                                           ncol = 1)
            ...
            ## update plots_row
            fit_tbl[[ curr_row, "plots"]] <- plots_row
...
if(FALSE) {
    ...
    fit_tbl <- fit_tbl_bkup
}


fit_tbl <- add_column_safely(fit_tbl, c("stats_fit", "fit_best", "fit_top"))

n_chains_max <- 4

for(curr_row in 1:nrow(fit_tbl)) {

    cat(paste("Row: \n", curr_row))

    fit_row <- fit_tbl[curr_row, ]
    fit_brms <- fit_row$fit[[1]]
    if(!is_brmsfit(fit_brms) ) {
        print("Fit slot missing brmsfit object, skipping...")              
    } else {
        ## fit == brmsfit so proceed
        fit_stan <- fit_brms$fit
        ...
        plot_violin <- mcmc_violin(fit_array, pars = c("lp__"), probs = c(0.1, 0.5, 0.9))
        if(display_plots) print(last_plot())
        stats_fit <- calc_fit_lp_stats(fit_stan)
        fit_tbl[[curr_row, "stats_fit"]][[1]] <- stats_fit
        ...
        fit_top$fit <- fit_stan_top
        ## update tbl
        fit_tbl[[curr_row, "fit_top"]][[1]] <- fit_best        
        
    }  
}

## Save a version of tbl 
fit_tbl_unfiltered <- fit_tbl
