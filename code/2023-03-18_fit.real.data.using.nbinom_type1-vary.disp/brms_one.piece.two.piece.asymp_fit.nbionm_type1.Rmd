                ...
                for(y0_flag in flags_y0_used) {
                    ...
                    for(disp_value in values_disp_used) {
                        ...
                        if(run_fits) {
                            ...
                            data <- data_stan
                            if(male_filter) data <- data %>% filter(!(male %in% male_filter))
                            ...
                            fit_stats <- calc_fit_lp_stats(fit_brms)
                            print(fit_stats)
                            fit_tbl[[curr_row, "fit_stats"]][[1]] <- fit_stats
...
fit_tbl <- add_column_safely(fit_tbl, c("lp_stats", "fit_best", "fit_top"))
...
for(curr_row in 1:nrow(fit_tbl)) {
    ...
    if(!is_brmsfit(fit_brms) ) {
        ...
        plot_violin <- mcmc_violin(fit_array, pars = c("lp__"), probs = c(0.1, 0.5, 0.9))
        last_plot()
        fit_stats <- calc_fit_lp_stats(fit_stan)
        fit_tbl[[curr_row, "fit_stats"]][[1]] <- fit_stats
