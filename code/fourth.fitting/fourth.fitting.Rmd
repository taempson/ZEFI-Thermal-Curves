...
- Look for order effects in round 1 and 2? 
  ...
  Seems like there will be an issue if we throw out, say, first two trials, when using `song_prop`.
...
temp_ref = 45 ## define a reference point for quadratic fits

xlab = "temp_mean - 45C"


plot_tmp <- ggplot(data) + 
    aes(x = (temp_mean), ## offset
        y = song_prop) +
#    ylim(0, 0.6) + 
    facet_wrap("male", scales = "free_y") +
    geom_point()

plot_glm_temp <- plot_tmp +
...
plot_glm_vpd_weighted_temp_max <- ggplot(data) + 
    aes(x = (temp_mean - 45),
        y = song_prop) +
    facet_wrap("male", scales = "free_y") +
    geom_point() + 
...
- Offsetting seems to have a minimal impact.
...
glmer_poisson_linear <- update(glmer_poisson, formula = formula_linear_RE,
                               verbose = 1)
...
- Output includes 
  ...
  > - Rescale variables?
  To me this suggests we should use `song_prop` and the glmer weights function.
  
  

#### Focus on promising models from above and don't filter data

```{r}

vec_temp_ref <- c(35, 44, 45)
...
fit_db <- expand_grid(temp_ref = vec_temp_ref, model = c("glm", "glmer"), dist = c("poisson", "qpoisson", "nb")) %>% 
    filter( !(model == "glm" & dist =="nb")) %>%
    filter( !(model == "glmer" & dist =="qpoisson")) %>%
    add_column(fit = list(list()) )
...
for(temp_ref in vec_temp_ref ){
    ...
    fit_db[fit_db$temp_ref==temp_ref &  fit_db$model == "glm" &  fit_db$dist == "poisson", ]$fit  #<- glm_poisson
    ...
    fit_db[fit_db$temp_ref==temp_ref &  fit_db$model == "glm" &  fit_db$dist == "qpoisson", fit] <- glm_qpoisson
    ...
    fit_db[fit_db$temp_ref==temp_ref &  fit_db$model == "glmer" &  fit_db$dist == "poisson", "fit"] <- glmer_poisson
    ...
    fit_db[fit_db$temp_ref==temp_ref &  fit_db$model == "glmer" &  fit_db$dist == "nb", "fit"] <- glmer_nb
