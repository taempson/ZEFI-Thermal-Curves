...
match_tbl_row <- function(filter_male = FALSE, x0_flag = "individual", y0_flag = "individual", disp_flag = "uniform_1", disp_value = 0.01, model = "two_piece", tbl = fit_tbl) {
...
if(run_fits) {
  ...
  outfile_tbl <- file.path(output_dir, paste0("fit_tbl_", cur_time, ".Rda"))
} else {
...
flags_x0_used <- c("individual", "groups_1", "uniform_1")[2] #
      ...
      for(x0_flag in flags_x0_used) {
        ...
        x0_label <- ifelse(model == "one_piece", "NA", x0_flag)
          ...
          for(disp_value in values_disp_used) {
            ...
            curr_row <- match_tbl_row(male_filter,
                                      x0_flag,
                                      y0_flag,
                                      disp_flag,
                                      disp_value,
            ...
            if(is.na(list(fit))) {
              ...
              if(model == "one_piece" & disp_flag != "individual") {
                ...
                for(male in males_used) {
                  ##print(male);
                  list_plot[[male]] <- pairs(fit, variable = c(male, "lp__"), regex = TRUE) 
                                                            # cex = 1,
                                                            # pch = ".") #+
