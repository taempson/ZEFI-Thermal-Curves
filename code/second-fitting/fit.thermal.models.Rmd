...
data_raw[[3]] <-read.csv(file.path(git_root, "data", "raw_data", "HSPi-Round-2-Heat-Trials.csv")) %>%
    mutate(round = 3) #%>%
#    group_by(male) %>% mutate(total_count_round_3 = sum(song_count) )
...
data_full <- full_join(data_raw[[1]], data_raw[[2]]) %>%
    ...
    group_by(male, round ) %>% mutate(count_total_round = sum(song_count) ) %>%
    ungroup() %>%
...
hist(data$count_total_round, breaks = 20)
```
...
data_summarize <-
    data %>% group_by(temp_target) %>%
    summarize(song_mean = mean(song_count),
              song_sd = sd(song_count),
              song_n= dplyr::n(),
...
plot_combine <- ggplot(data) +
                  aes(x = temp_target, y = song_count, color = male) +
                  geom_point()
plot_combine
...
#    labs(title = "Mean(song_count) vs. Target Temp",
#         subtitle = "with SE bars")
plot_summarize
              
plot_combine + plot_summarize
...
pdf("temp_mean-vs-humidity_mean.pdf")
plot_data_humidity_vs_temp
dev.off()
```
...
Clearly we want to work with `temp_mean`, rather than `temp_target` and, as a result, only round = 3 since we don't have this data for the other rounds.

## Compare song_count to `temp_mean` and `temp_humidity`
```{r}


p <- ggplot(data, aes(x = temp_mean, y = song_count), color = male) +
    geom_point() +
    stat_cor()
last_plot()

p <- ggplot(data, aes(x = humidity_mean, y = song_count), color = male) +
    geom_point() +
    stat_cor()
last_plot()
...
### GLM analysis of `song_count` and `temp_mean` for round 3 only
