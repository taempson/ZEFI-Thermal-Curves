...
flags_disp_used <- c("individual", "groups_1", "uniform_1", "uniform_2")[4] #  |> rev() |> first()
                ...
                for(y0_flag in flags_y0_used) {
                    ...
                    for(disp_value in values_disp_used) {
                        ...
                        if(run_fits) {
                            ...
                            for(flag in c("x0", "y0", "disp") ) {
                                switch_male <- which_switch_male(flag)

                                group_txt <- paste0(flag, "_group")
                            ...
                            print("Define model formulas for parameters")
                        ...
                        if(print_prior_summary) {
                            ...
                            for(fit_string in NULL) { #c("fit_top", "fit_best")) {
                                fit <- fit_tbl[[curr_row, fit_string]][[1]]
                                fit_stan <- fit$fit
                                data <- fit$data
                                males_used <- unique(data$male)
                            ...
                            plot_trace <- mcmc_trace(fit, pars = c("lp__")) +
                                ggtitle(desc_short_tmp)
                            print(plot_trace)
                            filename <- paste0("trace_", desc_filename_tmp, ".pdf")
                            ...
                            print(plot_violin)
                            filename <- paste0("violin_", desc_filename_tmp, ".pdf")
                            ggsave(filename = filename, path = file.path(output_dir, "figures"),
                                   width = 8, height = 11, units = "in",
                                   scale = 0.4,
                                   dpi=300)
                            ...
                            filename <- paste0("histogram_", desc_filename_tmp, ".pdf")
                            ggsave(filename = filename, path = file.path(output_dir, "figures"),
                                   width = 8, height = 11, units = "in",
                                   scale = 0.4,
                                   dpi=300)
                            ...
                            if(print_pairs_plot) {
                                ...
                                if(model == "one_piece" & disp_flag != "individual") {
                                    ...
                                    print(grid_plot)
                                    filename <- paste0("grid.plot_", desc_filename_tmp, ".png")
                                    ggsave(filename = filename, path = file.path(output_dir, "figures"),
                                           device = agg_png, 
                                           width = 8, height = 11, units = "in",
                                           scale = 0.4,
                        ...
                        } # end looping over fit_top and fit_best
                        curr_row <- curr_row + 1
                    }  ## End disp_values
                } ## end y0_flag
            } ## end x0_flag
