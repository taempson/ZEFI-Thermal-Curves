...
if(FALSE) {
  ...
  fit_tbl <- tibble(model = NA,
                    #sampling_dist = sampling_dists,
                    x0_flag = NA,
                    y0_flag = NA,
                    disp_flag = NA,
                    disp_value = NA,
                    desc = NA, #"NA_character",
                    filter_male = NA,
                    #y0_group_list = list(NA), #tbl_tmp, #list(NA),
                    #                      x0_group_list = list(NA),
                    fit = list(NA),
        ...
        for(y0_flag in flags_y0_used) {
          ...
          for(disp_value in values_disp_used) {
            ...
            plot_trace_and_violin <- plot_grid(plotlist = list(plot_trace, plot_violin),
                                           ncol = 1)
            if(save_plots) last_plot_save(file_prefix = "trace-and-violin")
            if(display_plots) print(last_plot())
            ...
            if(save_plots) last_plot_save(file_prefix = "histogram")            
            if(display_plots) print(last_plot())
            ...
            if(print_pairs_plot) {
              ...
              if(model == "one_piece" & disp_flag != "individual") {
                ...
                if(display_plots) print(last_plot())
                if(save_plots) last_plot_save(file_prefix = "grid.plot")

              } #end else for grid plot
            } # end print_plot_pairs
            curr_row <- curr_row + 1
          }  ## End disp_values
        } ## end y0_flag
      } ## end x0_flag
