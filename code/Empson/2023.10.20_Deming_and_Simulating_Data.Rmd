...
sample = 1000 # number of bootstrap and simulated samples
...
filtered.data %>% 
  filter(male=="T231") %>% 
  ggplot(aes(x=test_order, y=motif_count))+
  geom_point(color="black")+
  theme_minimal()+
  labs( x="Test Order", y="Number of Motifs", title="Does order explain the variation in the number of songs for T231? Nope.")
```
...
boot_male <-function(one_male, data.from,boot_set){
  ...
  for(i in 1:sample){ #Check the first code chunk for what sample is
    bootstrap_sample <- sample(data$motif_count,n, replace = TRUE) #Check the first code chunk for what n is
...
mean(testing.bootstrap.T229$a_mean) #187.731
mean(testing.bootstrap.T229$b_var) #7378.622
...
mean(testing.bootstrap.T236$a_mean) #201.2933
mean(testing.bootstrap.T236$b_var) #6892.865
```

# Running Bootstrap Function
    - Not sure why the function isn't running.
```{r}
simply_boot <- boot_male(one_male=unique.male, data.from=filtered.data, boot_set=out)
...
# Deming
```{r}
x <- filtered.data$observed_mean # Observed Mean for each male
y <- filtered.data$observed_var  # Observed Var for each male
xstd <- simply_boot$sd_boot_mean # Bootstrapped sd(Mean) for each male
ystd <- simply_boot$sd_boot_var # Bootstrapped sd(Var) for each male
...
# Running Simulation
```{r}
full.simulated.results <- simulating.for.boot(unique.male, out)
...
# Cleaning for graph
```{r}
simply_observed <- simply_observed %>%
