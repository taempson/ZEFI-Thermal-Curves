--
title: "Piecewise Regression using BRMS"
...
- Fit one and two piece poisson, quasiposson, and negative binomial using BRMS 

## Recap

- was able to fit two piece poisson using rstan, but
- data is overdispersed
- plotting of model and predictions was limited.
...
- see discussions at 
1. Piecewise Models: (https://discourse.mc-stan.org/t/piecewise-linear-mixed-models-with-a-random-change-point/5306/9)
2. Overdispersed poisson: (https://solomonkurz.netlify.app/post/2021-07-12-got-overdispersion-try-observation-level-random-effects-with-the-poisson-lognormal-mixture/)
...
file_basename <- "two.piece_poisson"
fit_file <- file.path(output_dir, paste0(file_basename, ".Rda"))
stan_file <- file.path(output_dir, paste0(file_basename, ".stan"))
...
data <- tibble(x = x) %>%
  mutate(lambda = if_else(x < 15, b0, b0 - b2 * (x - b1))) %>%
  mutate( y = rpois(length(x), lambda))
...
formula <- bf(y ~ fmax(0.01, b0 * (1 - step(x - b1) * b2)),
              b0 ~ 1,
              b1 ~ 1,
              b2 ~ 1,
              nl = TRUE )

##y0 * step(temp - 40) * (-(temp - 40)/(46 - 40)))

prior <- prior(uniform(30, 200), nlpar = "b0", lb = 30, ub = 200) +
  prior(uniform(1, 19), nlpar = "b1", lb = 1, ub = 19) +
  prior(uniform(1, 100), nlpar = "b2", lb = 5, ub = 50)
...
,
 ...
 prior <- prior(uniform(), nlpar = y0, lb = 0, ub = 1000) +
  prior(uniform(), nlpar = t0, ub = 45.9, lb = 30) +
  prior(constant(46), nlpar = tmax)
...
if(!file.exists(fit_file) | refit_model) {
...
nlform <-
  bf(song_count ~ rmax*exp(-0.5*(fabs(temp - topt)/a)^b),
     rmax ~ male -1,
     topt ~ 1,
     a ~ 1,
     b ~ 1,
