...
glm_combined <- glm(song_count ~ 1 + temp_target + I(temp_target^2),
                    family = quasipoisson(),
...
## What the heck, try sqrt...

pdf(file = "song_count_vs_temp_all-2.pdf")
ggplot(data_full) + 
    aes(x = temp_target,
        y = song_count,
        color = (round == 1)) +
    ...
    labs(x = xlab, y = ylab,
         subtitle = "glm:y ~poisson(1 + x + x^2)",
         title = "Song Count vs Temp: Poisson Regression") 
...
comment(fit_tbl) <- "temp_slope = temp_target, temp_curve = I(temp_target^2)"
...
glm$poisson <- glm(song_count ~ 1 + male + trial_index + temp_target + I(temp_target^2), data = data_r3, family = poisson(link = "log"))
