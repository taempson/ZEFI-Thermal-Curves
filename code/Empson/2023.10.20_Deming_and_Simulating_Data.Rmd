...
```{r message=FALSE}
setwd("/Users/taraempson/Documents/GitHub/ZEFI-Thermal-Curves/code/Empson")
library(tidyverse)
library(janitor)
library(deming)
library(patchwork)
...
setwd("../../data/collated/")
set.seed(1)
og.raw.data <- read.csv("HSPi-Repeatability-Song-Count.csv")
raw.data <- og.raw.data %>% 
    filter(male!="T230")
raw.data$day<- c(1,2,3,1,2,3,1,2,3,1,2,3,1,2,3,1,2,3,1,2,3,1,2,3,1,2,3)
#adding in day order
...
simplified_real <- raw.data %>% 
    ...
    select(-chamber, -date, -motif_count, -counter, -temp_target, -temp_mean, -temp_median, -humidity_mean, -day)
...
real <- og.raw.data %>% 
    ...
    mutate(real_sd=sd(motif_count)) %>% 
     mutate(real_mean=mean(motif_count)) %>% 
    mutate(real_var=var(motif_count)) %>% 
    select(-chamber, -date, -motif_count, -counter, -temp_target, -temp_mean, -temp_median, -humidity_mean)
real <- unique(real)
```
