...
- Thus, given a flat prior on $b0$, the corresponding prior on $x0$ is $f(x0) = C \frac{1}{(x_{\max} - x0)^2}$
...
models <- c("piecewise", "b0_piecewise") #, "asymptotic")
  ...
  for(x0_flag in flags_x0_used) {
    for(y0_flag in flags_y0_used) {
      for(disp_prior in disp_prior_used) {
        ...
        desc <- paste0(model, "; ", sampling, "; ", desc_short)
...
##                     )
          ...
          hist <- stan_hist(fit_stan, pars = c("b_y0", "disp"), bins = 25) +
            ggtitle(desc_short)
          print(hist)
          ## Save hist to file
          hist <- stan_hist(fit_stan, pars = c("b_x0", "disp"), bins = 25)  +
            ggtitle(desc_short)
          print(hist)
          ## Save hist to file
