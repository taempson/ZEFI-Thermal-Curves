...
print(comp)
print(fit_tbl$desc, width = 10)

```
Overall,

- `loo` suggests the model is misspecified since `p_loo` $>>$ the number of actual model parameters (based on LOO FAQ document).
- Some data points are problematic given their $\hat{k}$ diagnostics.

Note, also

- Reducing 
  - `sd_y0_prior` from  1000 to 200 
  - `alpha_theta_prior` from 40 to 10
  Results in
  - Improved `p_loo` values, but still much greater than the number of parameters.
  - Smaller elpd_diff values and swapping of model 4 and 7's ranking from 1, 7, 4 to 1, 4, 7.
...
cmodel <- cmdstan_model(stan_file = "two.piece_nb_1.0.stan")

stan_model(file="two.piece_nb_1.0.stan",
           verbose = TRUE)
