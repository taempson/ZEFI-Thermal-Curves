...
for(row_index in 1:nrow(fit_tbl)) {
  ...
  print_mcmc_pairs <- FALSE # individual level parameters
  ...
  if(all(unique(male_instance) == 2)) {
    ...
    if(print_mcmc_pairs) {
      ...
      p <- plot_grid(plotlist = pairs_list,
                     ncol = 3)
      plot_pairs_list <- plot_grid(title_row, p,  ncol = 1, rel_heights=c(0.1, 1))
  ...
  if(length(vars_tmp) > 0 ) stan_plot(fit_stan_rename, pars = vars_tmp) +
    ggtitle("Thresholds", subtitle = desc)
