...
source("../Local.Functions/local.functions_ZFI.fittings.Rmd")
...
### Print Posterior Probability Traces and Identify Best Chains When Need

- Should probably move some of this code to the fitting routines.
...
for(row_index in 1:nrow(fit_tbl)) {
  ...
  if(n_chains != n_chains_best) {
    print(paste("Chains Best:", chains_best, " (", n_chains_best, "out of", n_chains, ")" ))
    print("Dropping other chains")
  }

  if(n_chains_best > n_chains_target) {
    ## just take the first n target Chains
    print("Dropping extra chains")
    chains_best <- chains_best[1:n_chains_target]
  }

  ## Keep only desired chains
  fit_brms_best <- keep_chains(fit_brms, chains_best)
  ## update tbl
  fit_tbl[[row_index, "fit"]] <- fit_brms_best
}
