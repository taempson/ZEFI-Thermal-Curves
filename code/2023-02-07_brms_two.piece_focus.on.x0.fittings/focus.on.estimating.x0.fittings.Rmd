...
- Having set `x0max` to 45.9, we get 95% PI that are below this point.
...
y0_hist <- data_ind %>% select(y0_simple_est) %>% unique() %>% ggplot(aes(y0_simple_est)) + geom_histogram(bins = 10)
print(y0_hist)
...
summary_stats <- data_ind %>% ungroup() %>%  summarize(y0_bar = mean(y0_simple_est), y0_sd = sd(y0_simple_est), phi_bar = median(phi_ind), theta_bar = y0_bar^2/(y0_sd^2 - y0_bar))
...
#models <- c("brms")
sampling_dists <- c("nb", "lognormal", "com_poisson") 
flags_x <- c("uniform_1", "groups_1", "individual")
...
## Run Analysis
for(sampling in sampling_dists[[3]]) {
    ...
    for(y_flag in flags_y) {
      ...
      fit_tbl[[curr_row, "y0_group_list"]] <- list(unique(data[, c("male", "y0_group")]))
        ...
        family <- switch(sampling,
                         "nb"= negbinomial(link = "identity", link_shape = "identity"),
                         "lognormal" = lognormal(link = "identity", link_sigma = "log"),
                         ## generalized poisson
                         ## quasipoisson
                         ...
                         "com_poisson"=brmsfamily("com_poisson", link="identity", link_shape="log") #com_poisson(link="identity", link_shape="log")
                         )
      prior_core <- prior(
      ...
      nlform <- bf(
        ...
        save_model <-paste0(paste("brms", sampling, x_flag, y_flag, sep="_"), ".stan")
    ...
    stanfit <- fit
        ...
        print(fit)
        save(file = file.path(output_dir, paste0(save_model, ".Rda-tmp")), fit)
...
save(file = file.path(output_dir, paste0("fit_tbl_", format(Sys.time(), "%Y-%m-%d_%H-%M"), ".Rda")), fit_tbl)
