---
title: " Visualizing `brms` fits to simulated data using tidybayes."
author: "Michael Gilchrist"
date: "Created: 2023-01-31; Code run on `r Sys.Date()`"
output: 
...
This file was derived from the `tidybayes` vignette `tidy-brms.Rmd`
...
options(mc.cores = min(1, (parallel::detectCores()-2)))
...
## Taken from `brms_two.piece_create.and.fit.simulated.data.Rmd`
...
Simulated data is based on these values

```{r load_summary_stats}

load(file.path(input_dir, "obs_summary_stats.Rda"), verbose = TRUE)

xx_means <- c(39-2, 39+2) ## values based on mean = 39, sd = 1 and n_sd = 2
yy_means <- summary_stats$y0_bar + c(-2, 2)*summary_stats$y0_sd


```

### Simulated Data and Model Fits

```{r load_data_and_fits}

x_flag_data <- "groups_2"
y_flag_data <- "groups_2"


input_tmp <- file.path(input_dir, c("data_sim_tbl", paste0("fit_tbl_data_", x_flag_data, "_", y_flag_data, "_n32.Rda")))
                       
print(input_tmp)

## need to set envir or else data loaded into the immediate `parent.frame()`
## Here that's the `walk()` function's environment.
walk(input_tmp,  ~ load(.x, envir = .GlobalEnv, verbose = TRUE))

data <- data_sim %>% filter(x_flag == x_flag_data & y_flag == x_flag_data, sampling_dist =="nb", n_reps == 32) ## could try to add column with strings for faced_grid labelling mutate(factor(xx, labels = c("mean(x_0) =  
```
...
data_parms <- data[, c("index", "x0", "xx", "y0", "yy")] %>% unique()

p <- ggplot(data_parms)b

hist_x0_vals <- p + 
  aes(x0, fill = factor(xx)) +
  geom_histogram(binwidth = 0.75)
last_plot()
hist_y0_vals <- p +
  aes(y0, fill = factor(yy)) +
  geom_histogram(binwidth = 0.75)
last_plot()

scatter_x0_vs_y0 <- p +
  aes(x = x0, y = y0, color = interaction(xx, yy, sep=':')) + 
  geom_point() +
...
plot <- ggplot(data) +
  aes(y = motif_count, x = temp_mean, color = factor(index)) +
  geom_point() +
  facet_grid(rows = vars(xx), cols = vars(yy), scales = "free_y")
last_plot()
```
