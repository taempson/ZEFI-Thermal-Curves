...
pars <- c("y0", "x0")
plot(model, pars = c("x0"))
plot(model, pars = c("y0"))


pars_est <- summary(model, pars = pars, probs = NULL)$summary %>%
                                                    as_tibble(rownames = "par")

pars_est %>% separate_wider_regex(cols = par, c(pars = "^[a-z]+", "[", male = "[0-9]+", "]"))

pars_sim <- data_sim %>% select(male, y0, tau0)


fit_poisson_pooled <- model
