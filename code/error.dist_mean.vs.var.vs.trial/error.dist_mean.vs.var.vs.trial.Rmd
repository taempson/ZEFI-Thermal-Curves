...
stats_40 <- data_40 %>%
    group_by(male) %>%
    summarize(mean = mean(song_count),
              var = var(song_count, na.rm = TRUE),
              cv = sqrt(var)/mean,
              count = length(song_count), ) %>% mutate()
...
for(filter_high_cv in c(FALSE, TRUE)){
    for(filter_low_mean in c(FALSE, TRUE)){
        ...
        ga <- grid.arrange(g2, g3, g4, as_ggplot(legend),
                           ncol=2,
                           top=textGrob(
                               paste0("song_count vs. trial: Filter High CV = ", filter_high_cv, ", Low Mean = ", filter_low_mean, "\n temp_target = 40C"),
                               gp=gpar(fontsize = 11))
                           )
        ##cat("\n\n\\pagebreak\n")
        ##print("<P style='page-break-before: always'>")    #forced new-page happens here.
        ...
        dev.print(device = pdf, file = file.path(output_dir, paste0("song_count.vs.trial_filter.high-", filter_high_cv, ".filter.low.mean-", filter_low_mean, ".pdf")))
    }
}
