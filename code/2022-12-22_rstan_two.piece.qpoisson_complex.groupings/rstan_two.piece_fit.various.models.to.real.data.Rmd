...
if(filter_data) {
  ...
  males_filtered_mean <- motif_stats %>%
    filter(mean > 20) %>% # changing to 20 removes previous male 7
    pull(male)
...
y0_group <- list()
y0_group[[1]] <- c(1, 5, 6, 9, 10)
y0_group[[2]] <- c(2, 3, 4, 7, 8)
...
x0_group_index <- map_int(data$male, ~ if_else(. %in% x0_group[[1]], 1, 2))
y0_group_index <- map_int(data$male, ~ if_else(. %in% y0_group[[1]], 1, 2))
