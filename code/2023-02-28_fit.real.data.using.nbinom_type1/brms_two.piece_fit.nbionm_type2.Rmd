  ...
  for(x0_flag in flags_x0_used) {
    for(y0_flag in flags_y0_used) {
      for(disp_prior in disp_prior_list) {
        ...
        desc <- paste0(model, ": ", sampling, "; x0 " , x0_flag, "; y0 ", y0_flag, ", disp prior: ", round(disp_prior,2))
        ...
        save_model <-paste0(paste("stan/brms", model, sampling, x0_flag, y0_flag, disp_prior, "_"), ".stan")
...
save(file.path(output_dir, paste0("fit_tbl_", Sys.Date() , ".Rda")), fit_tbl)
